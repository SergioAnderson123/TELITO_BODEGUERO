...
public class LoteReporteServlet extends HttpServlet {
    ...
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Object[]> listaLotes = loteDao.listarLotesPorProductor(idProductor);

        String filtrosInfo = "Todos los lotes";

        String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String nombreArchivo = "Reporte_Lotes_Productor_" + fecha + ".xlsx";
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/productor/enviar-reporte-lotes.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response)
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/productor/LoteReporteServlet?action=formEnviar");
            return;
        }
        ...
        try {
            ...
            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ?
                asunto : "Reporte de Lotes - Productor - TELITO BODEGUERO";

            // Calcular estadísticas
            int totalLotes = listaLotes.size();
            int totalStock = 0;
            for (Object[] lote : listaLotes) {
                if (lote[4] != null) {
                    totalStock += (Integer) lote[4]; // stock_actual
                }
            }
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>📦 Reporte de Lotes - Productor</h2>
                        </div>
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de Lotes generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>
        ...
        response.sendRedirect(request.getContextPath() + "/ProductorServlet?action=formRegistrarLote");
    }
}
