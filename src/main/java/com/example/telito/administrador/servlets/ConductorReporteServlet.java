...
/**
 * Servlet para manejar la exportación de reportes de conductores a Excel
 * y el envío de estos reportes por correo electrónico.
 */
@WebServlet(name = "ConductorReporteServlet", value = "/administrador/ConductorReporteServlet")
public class ConductorReporteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (!AuthorizationHelper.puedeAccederAdministrador(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de administrador intentó acceder a ConductorReporteServlet desde: " + 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        HttpSession session = request.getSession(false);
        if (!AuthorizationHelper.puedeAccederAdministrador(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de administrador intentó acceder a ConductorReporteServlet (POST) desde: " + 
                             request.getRemoteAddr());
            String redirectUrl = AuthorizationHelper.obtenerUrlRedireccionPorRol(session, request.getContextPath());
            response.sendRedirect(redirectUrl);
            return;
    ...
    /**
     * Exporta la lista de conductores a Excel y la descarga.
     */
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response) 
        ...
        ArrayList<Conductor> listaConductores = conductorDAO.listarTodosConductores();
        
        // Construir texto de filtros aplicados (para conductores no hay filtros por ahora)
        String filtrosInfo = "Todos los conductores";
        
        // Generar nombre de archivo con fecha
        String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String nombreArchivo = "Reporte_Conductores_" + fecha + ".xlsx";
        ...
        try (OutputStream out = response.getOutputStream()) {
            ExcelUtil.generarExcelConductores(listaConductores, out, filtrosInfo);
            out.flush();
        } catch (Exception e) {
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response) 
        ...
        request.getRequestDispatcher("/administrador/enviar-reporte-conductores.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/administrador/ConductorReporteServlet?action=formEnviar");
            return;
        }
        ...
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(tempFile)) {
                ExcelUtil.generarExcelConductores(listaConductores, fos, filtrosInfo);
                fos.flush();
            }
            
            // Preparar mensaje HTML
            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ? 
                asunto : "Reporte de Conductores - TELITO BODEGUERO";
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de conductores generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>
                            
                            <div class="info-box">
                                <h3 style="margin-top: 0; color: #006d77;">📋 Información del Reporte</h3>
                                <p><strong>Total de conductores:</strong> %d</p>
        ...
        // Redirigir a la gestión de conductores
        response.sendRedirect(request.getContextPath() + "/administrador/ConductorServlet?action=listar");
    }
}
