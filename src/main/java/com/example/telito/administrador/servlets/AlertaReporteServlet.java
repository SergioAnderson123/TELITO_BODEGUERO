...
/**
 * Servlet para manejar la exportación de reportes de alertas a Excel
 * y el envío de estos reportes por correo electrónico.
 */
@WebServlet(name = "AlertaReporteServlet", value = "/AlertaReporteServlet")
public class AlertaReporteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (!AuthorizationHelper.puedeAccederAdministrador(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de administrador intentó acceder a AlertaReporteServlet desde: " + 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        HttpSession session = request.getSession(false);
        if (!AuthorizationHelper.puedeAccederAdministrador(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de administrador intentó acceder a AlertaReporteServlet (POST) desde: " + 
                             request.getRemoteAddr());
            String redirectUrl = AuthorizationHelper.obtenerUrlRedireccionPorRol(session, request.getContextPath());
            response.sendRedirect(redirectUrl);
            return;
    ...
    /**
     * Exporta la lista de alertas a Excel y la descarga.
     */
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response) 
        ...
        ArrayList<AlertaConfig> listaAlertas = alertaDAO.listarTodasAlertas();
        
        // Construir texto de filtros aplicados
        String filtrosInfo = "Todas las alertas";
        
        // Generar nombre de archivo con fecha
        String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String nombreArchivo = "Reporte_Alertas_" + fecha + ".xlsx";
        ...
        try (OutputStream out = response.getOutputStream()) {
            ExcelUtil.generarExcelAlertas(listaAlertas, out, filtrosInfo);
            out.flush();
        } catch (Exception e) {
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response) 
        ...
        request.getRequestDispatcher("/administrador/enviar-reporte-alertas.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/AlertaReporteServlet?action=formEnviar");
            return;
        }
        ...
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(tempFile)) {
                ExcelUtil.generarExcelAlertas(listaAlertas, fos, filtrosInfo);
                fos.flush();
            }
            
            // Preparar mensaje HTML
            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ? 
                asunto : "Reporte de Alertas - TELITO BODEGUERO";
            
            // Contar alertas activas e inactivas
            long alertasActivas = listaAlertas.stream().filter(AlertaConfig::isActivo).count();
            long alertasInactivas = listaAlertas.size() - alertasActivas;
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>⚠️ Reporte de Alertas</h2>
                        </div>
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de alertas generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>
                            
                            <div class="info-box">
                                <h3 style="margin-top: 0; color: #006d77;">📋 Información del Reporte</h3>
                                <p><strong>Total de alertas:</strong> %d</p>
                                <p><strong>Alertas activas:</strong> %d</p>
                                <p><strong>Alertas inactivas:</strong> %d</p>
        ...
        // Redirigir a la gestión de alertas
