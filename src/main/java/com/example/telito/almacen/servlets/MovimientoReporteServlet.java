...
public class MovimientoReporteServlet extends HttpServlet {
    ...
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Movimiento> listaMovimientos = movimientoDao.listarTodosMovimientos();

        String filtrosInfo = "Todos los movimientos";

        String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String nombreArchivo = "Reporte_Movimientos_Almacen_" + fecha + ".xlsx";
        ...
        try (OutputStream out = response.getOutputStream()) {
            ExcelUtil.generarExcelMovimientosInventario(listaMovimientos, out, filtrosInfo);
            out.flush();
        } catch (Exception e) {
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/almacen/enviar-reporte-movimientos.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response)
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/almacen/MovimientoReporteServlet?action=formEnviar");
            return;
        }
        ...
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(tempFile)) {
                ExcelUtil.generarExcelMovimientosInventario(listaMovimientos, fos, filtrosInfo);
                fos.flush();
            }

            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ?
                asunto : "Reporte de Movimientos - Almacén - TELITO BODEGUERO";
            ...
            for (Movimiento mov : listaMovimientos) {
                ...
                if (tipo != null) {
                    if (tipo.equalsIgnoreCase("Entrada")) {
                        totalEntradas++;
                        cantidadEntradas += mov.getCantidad();
                    } else if (tipo.equalsIgnoreCase("Salida")) {
                        totalSalidas++;
                        cantidadSalidas += mov.getCantidad();
                    } else if (tipo.equalsIgnoreCase("Ajuste")) {
                        totalAjustes++;
                        cantidadAjustes += mov.getCantidad();
                    }
                }
            }
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>📦 Reporte de Movimientos - Almacén</h2>
                        </div>
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de Movimientos de Inventario del Almacén generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>
                            ...
                            <div class="info-box">
                                ...
                                <div class="stats-box">
                                    <strong>Entradas:</strong> %d movimientos (%d unidades)
                                </div>
                                <div class="stats-box">
                                    <strong>Salidas:</strong> %d movimientos (%d unidades)
                                </div>
                                <div class="stats-box">
                                    <strong>Ajustes:</strong> %d movimientos (%d unidades)
                                </div>
                                <div class="stats-box">
        ...
        response.sendRedirect(request.getContextPath() + "/almacen/MovimientoServlet");
    }
}
