...
public class PedidoReporteServlet extends HttpServlet {
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response)
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/almacen/PedidoServlet?action=listarPedidos");
            return;
        }
        ...
        try {
            ...
            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ?
                asunto : "Reporte de Pedidos de Salida - Almacén - TELITO BODEGUERO";
            ...
            for (Pedido pedido : listaPedidos) {
                String estado = pedido.getEstadoPreparacion();
                if ("Pendiente".equals(estado)) pendientes++;
                else if ("En preparación".equals(estado)) enPreparacion++;
                else if ("Listo para despacho".equals(estado)) listos++;
            }
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>📦 Reporte de Pedidos de Salida - Almacén</h2>
                        </div>
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de Pedidos de Salida del Almacén generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>

                            <div class="info-box">
                                <h3 style="margin-top: 0; color: #006d77;">📊 Información del Reporte</h3>
                                <div class="stats-box">
                                    <strong>Total de pedidos:</strong> %d
                                </div>
                                <div class="stats-box">
                                    <strong>Pendientes:</strong> %d
                                </div>
                                <div class="stats-box">
                                    <strong>En preparación:</strong> %d
                                </div>
                                <div class="stats-box">
                                    <strong>Listos para despacho:</strong> %d
                                </div>
                                <div class="stats-box">
                            ...
                            <p><strong>Nota:</strong> El archivo Excel incluye información detallada de cada pedido: número, cliente, destino, estado, fecha de creación y productos.</p>
