...
@WebServlet(name = "OrdenCompraReporteServlet", value = "/logistica/OrdenCompraReporteServlet")
public class OrdenCompraReporteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!AuthorizationHelper.puedeAccederLogistica(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de logística intentó acceder a OrdenCompraReporteServlet desde: " + 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession(false);
        if (!AuthorizationHelper.puedeAccederLogistica(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de logística intentó acceder a OrdenCompraReporteServlet (POST) desde: " + 
                             request.getRemoteAddr());
            String redirectUrl = AuthorizationHelper.obtenerUrlRedireccionPorRol(session, request.getContextPath());
            response.sendRedirect(redirectUrl);
            return;
    ...
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<OrdenCompraBean> listaOrdenes = ordenCompraDao.listarTodasOrdenes(busqueda, proveedorId, estado);
        ...
        if (proveedorId != null && !proveedorId.trim().isEmpty()) {
            ...
            filtrosInfo.append("Proveedor: ").append(proveedorId);
        }
        ...
        if (filtrosInfo.length() == 0) {
            filtrosInfo.append("Todas las órdenes");
        }

        String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        String nombreArchivo = "Reporte_OrdenesCompra_" + fecha + ".xlsx";
        ...
        try (OutputStream out = response.getOutputStream()) {
            ExcelUtil.generarExcelOrdenesCompra(listaOrdenes, out, filtrosInfo.toString());
            out.flush();
        } catch (Exception e) {
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response)
        ...
        if (proveedorId != null) {
            request.setAttribute("proveedor", proveedorId);
        }
        if (estado != null) {
            request.setAttribute("estado", estado);
        }

        request.getRequestDispatcher("/logistica/enviar-reporte-ordenes-compra.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response)
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            String redirectUrl = request.getContextPath() + "/logistica/OrdenCompraReporteServlet?action=formEnviar";
            if (busqueda != null) redirectUrl += "&busqueda=" + busqueda;
            if (proveedorId != null) redirectUrl += "&proveedor=" + proveedorId;
        ...
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(tempFile)) {
                ExcelUtil.generarExcelOrdenesCompra(listaOrdenes, fos, filtrosInfo.toString());
                fos.flush();
            }

            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ?
                asunto : "Reporte de Órdenes de Compra - TELITO BODEGUERO";

            // Calcular monto total
            double montoTotal = 0.0;
            for (OrdenCompraBean orden : listaOrdenes) {
                String montoStr = orden.getMontoTotal();
                if (montoStr != null && !montoStr.trim().isEmpty()) {
                    try {
                        String montoLimpio = montoStr.replace("S/.", "").replace(" ", "").trim();
                        montoTotal += Double.parseDouble(montoLimpio);
        ...
        String redirectUrl = request.getContextPath() + "/orden-compra";
        StringBuilder params = new StringBuilder();
        if (busqueda != null) params.append("?busqueda=").append(busqueda);
        if (proveedorId != null) params.append(params.length() == 0 ? "?" : "&").append("proveedor=").append(proveedorId);
