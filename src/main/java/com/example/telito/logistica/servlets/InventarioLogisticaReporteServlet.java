...
@WebServlet(name = "InventarioLogisticaReporteServlet", value = "/logistica/InventarioLogisticaReporteServlet")
public class InventarioLogisticaReporteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!AuthorizationHelper.puedeAccederLogistica(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de logística intentó acceder a InventarioLogisticaReporteServlet desde: " + 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession(false);
        if (!AuthorizationHelper.puedeAccederLogistica(session)) {
            System.err.println("🚨 ACCESO DENEGADO: Usuario sin rol de logística intentó acceder a InventarioLogisticaReporteServlet (POST) desde: " + 
                             request.getRemoteAddr());
            String redirectUrl = AuthorizationHelper.obtenerUrlRedireccionPorRol(session, request.getContextPath());
            response.sendRedirect(redirectUrl);
            return;
    ...
    private void exportarExcel(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<InventarioBean> listaInventario = inventarioDao.listarTodoInventarioAgrupado(busqueda, estado);
        ...
        String nombreArchivo = "Reporte_InventarioLogistica_" + fecha + ".xlsx";
        ...
        try (OutputStream out = response.getOutputStream()) {
            ExcelUtil.generarExcelInventarioLogistica(listaInventario, out, filtrosInfo.toString());
            out.flush();
        } catch (Exception e) {
    ...
    private void mostrarFormularioEnvio(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/logistica/enviar-reporte-inventario.jsp")
    ...
    private void enviarPorCorreo(HttpServletRequest request, HttpServletResponse response)
        ...
        if (emailDestino == null || emailDestino.trim().isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/logistica/InventarioLogisticaReporteServlet?action=formEnviar" +
        ...
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(tempFile)) {
                ExcelUtil.generarExcelInventarioLogistica(listaInventario, fos, filtrosInfo.toString());
                fos.flush();
            }

            String asuntoFinal = (asunto != null && !asunto.trim().isEmpty()) ?
                asunto : "Reporte de Inventario Logística - TELITO BODEGUERO";

            // Calcular valor total
            double valorTotal = 0.0;
            for (InventarioBean inventario : listaInventario) {
                valorTotal += inventario.getPaquetesDisponibles() * inventario.getPrecioPorPaquete();
            }
            ...
            String mensajeHTML = """
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>📦 Reporte de Inventario Logística</h2>
                        </div>
                        <div class="content">
                            <p>Estimado/a,</p>
                            <p>Se adjunta el reporte de Inventario Logística generado desde el sistema <strong>TELITO BODEGUERO</strong>.</p>
        ...
        response.sendRedirect(request.getContextPath() + "/InventarioServlet" +
            (busqueda != null ? "?busqueda=" + busqueda : "") +
            (estado != null ? (busqueda != null ? "&" : "?") + "estado=" + estado : ""));
    }
}
